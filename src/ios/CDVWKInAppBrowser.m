...
- (void)openInInAppBrowser:(NSURL*)url withOptions:(NSString*)options
{
    ...
    height = browserOptions.height ? browserOptions.height : frame.size.height;
    CGRect viewBounds = CGRectMake(originx, originy, width, height);

    _waitForBeforeload = ![_beforeload isEqualToString:@""];

    [self.inAppBrowserViewController navigateTo:url];
    if (!browserOptions.hidden) {
        [self show:nil withNoAnimate:browserOptions.hidden rect: viewBounds];
    }
}

- (void)show:(CDVInvokedUrlCommand*)command{
    CGRect bounds = [[UIScreen mainScreen] bounds];
    [self show:command withNoAnimate:YES rect: bounds];
}

- (void)show:(CDVInvokedUrlCommand*)command withNoAnimate:(BOOL)noAnimate rect:(CGRect)viewRect
{
    BOOL initHidden = NO;
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if (weakSelf.inAppBrowserViewController != nil) {
            ...
            if (!strongSelf->tmpWindow) {
                strongSelf->tmpWindow = [[UIWindow alloc] initWithFrame: viewRect];
            }
...
- (void) setWebViewFrame : (CGRect) frame {
    NSLog(@"NOT Setting the WebView's frame to %@", NSStringFromCGRect(frame));
